...
2. **Individual story summaries** - `docs/sprint-artifacts/completions/{{story}}-summary.md` (generated by Hermes for each story)
3. **Hermes artifacts** - `{{story}}-hermes.json` (contains TL;DR and quick stats)
4. **Progress artifacts** - `docs/sprint-artifacts/completions/{{story}}-progress.json`
...
cat docs/sprint-artifacts/completions/{{story}}-progress.json

# Builder artifact (has files created/modified)
cat docs/sprint-artifacts/completions/{{story}}-metis.json

# Inspector artifact (has task verification)
cat docs/sprint-artifacts/completions/{{story}}-argus.json

# Triage artifact (has issue breakdown)
cat docs/sprint-artifacts/completions/{{story}}-themis.json

# Playbook artifact (has learnings)
cat docs/sprint-artifacts/completions/{{story}}-mnemosyne.json
```
...
📄 **Full Report:** `docs/sprint-artifacts/completions/{{story}}-summary.md`
...
1. **{{Story 1}}** - {{Why verify first}}
   - Read: `docs/sprint-artifacts/completions/{{story1}}-summary.md`
   - Focus on: {{Key things to verify}}

2. **{{Story 2}}** - {{Why verify second}}
   - Read: `docs/sprint-artifacts/completions/{{story2}}-summary.md`
   - Focus on: {{Key things to verify}}
...
```bash
REPORT_FILE="docs/sprint-artifacts/session-reports/session-{{timestamp}}.md"
mkdir -p docs/sprint-artifacts/session-reports
```
...
```json
{
  ...
  "report_path": "docs/sprint-artifacts/session-reports/session-{{timestamp}}.md",
  "summary": {
...
Save to: `docs/sprint-artifacts/completions/session-{{timestamp}}-hermes.json`
