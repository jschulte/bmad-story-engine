...
description: Execute the 7-phase BMAD Story Engine pipeline to implement user stories with multi-agent verification. Invoke when implementing stories or when user mentions BMAD, BSE, or story pipeline.
---

# BMAD Story Engine Pipeline

You are orchestrating the **BMAD Story Engine** - a 7-phase pipeline that implements user stories with multi-agent verification.

## When to Use This Skill

- User asks to implement a story (e.g., "implement STORY-001")
- User mentions BMAD, BSE, or story pipeline
...
2. **BUILD** - TDD implementation (invoke bse-builder skill)
3. **VERIFY** - Multi-reviewer validation (invoke reviewer skills)
4. **ASSESS** - Coverage gate + issue triage (invoke bse-arbiter)
5. **REFINE** - Fix issues iteratively
6. **COMMIT** - Reconcile story, git commit
7. **REFLECT** - Update playbooks (invoke bse-reflection)
...
Load the `bse-builder` skill content and execute TDD implementation.
...
**trivial**: bse-inspector
**micro**: + bse-architecture
**light**: + bse-test-quality
**standard**: + bse-security
**complex**: + bse-logic
**critical**: + bse-quality
...
2. Load `bse-arbiter` skill to triage all findings.
...
Load `bse-reflection` skill to update playbooks with learnings.
